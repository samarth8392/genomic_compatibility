...
###                                                                     ###
###     Date Created: 09/02/22                  Last Modified: 09/24/23 ###
###########################################################################
###########################################################################
###                     02_provean.sh  				                    ###
###########################################################################
...
# Step1: Divide all missense mutations into smaller sets (N_missense = 69,262)

# cd $OUTDIR
# mkdir sites
# cd sites
# split -l 1000 -d $MAINDIR/sites/final172.missense.pos.txt snpSet
...
# .
# ├── output
# │   └── output_provean_final172.nonSyn.snpSet00.xlsx
# └── snpEff
#     ├── build.logfile.txt
#     ├── data
#     │   └── scat
...
#     ├── final172.allchr.nonSyn.SNPEff.vcf
#     ├── final172.allchr.Syn.SNPEff.vcf
#     ├── no-downstream
...
# 	cd results/
	# mkdir $i
	# cd $i
	# vcftools --vcf $SNPEFFDIR/final172.allchr.nonSyn.SNPEff.vcf \
	# --recode --recode-INFO-all \
	# --positions $OUTDIR/sites/snpSet$i \
	# --out final172.nonSyn.snpSet$i

	# mv final172.nonSyn.snpSet$i.recode.vcf final172.nonSyn.snpSet$i.vcf 
	# cp -lr $OUTDIR/ePat/ ./

	# mkdir final172.nonSyn.snpSet$i.vcf_dir
	# cd final172.nonSyn.snpSet$i.vcf_dir
...
# i=final172.nonSyn.snpSet$i.vcf
# SHARED_DIR=$OUTDIR/results/${r}/final172.nonSyn.snpSet${r}.vcf_dir/snpEff
# WORK_DIR=$OUTDIR/results/${r}/
# TMP_DIR=/tmp/final172.nonSyn.snpSet${r}.vcf
# ###############################################
# mkdir -p /tmp/final172.nonSyn.snpSet${r}.vcf 

# singularity run -B /tmp/final172.nonSyn.snpSet${r}.vcf:/root/tmp \
# -B $OUTDIR/results/${r}/final172.nonSyn.snpSet${r}.vcf_dir/snpEff:/root/snpEff \
# -B $OUTDIR/results/${r}/:$OUTDIR/results/${r}/ \
# -W $OUTDIR/results/${r}/ \
# $OUTDIR/results/${r}/ePat/ePat.sif \
# $OUTDIR/results/${r}/ePat/script/automated_provean.sh \
# -i final172.nonSyn.snpSet${r}.vcf -r scat 

# rm -rf /tmp/final172.nonSyn.snpSet${r}.vcf" \
...
# for i in {00..68}
# do
# 	cp results/$i/final172.nonSyn.snpSet${i}.vcf_dir/output/*xlsx final/xl/
# 	cp results/$i/final172.nonSyn.snpSet${i}.vcf_dir/output/*txt final/txt/
# done
...
final172.nonSyn.provean

# Get deleterious and neutral functional variation

cut -f1,2,182,183 final172.nonSyn.provean.txt | grep "D" | sort -k1,1 -k2,2n > final172.deleterious.pos.txt
...
cd $OUTDIR
cut -f1,2,8 final172.nonSyn.provean.txt > final172.nonSyn.annotate.txt
