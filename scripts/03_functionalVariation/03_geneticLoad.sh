...
###                                                                     ###
###     Date Created: 11/26/22                 Last Modified: 12/20/23  ###
###########################################################################
###########################################################################
###                     03_geneticLoad.sh                        		###
###########################################################################
...
do
	...
	do
		...
		--positions $MAINDIR/sites/final172.$type.pos.txt \
		--freq \
		--keep $MAINDIR/lists/byPop/${pop}.final172.sample.list \
		--out freq/${pop}.${type}
...
# 		for r in {1..100}
# 		do
# 			shuf -n 8 /fs/ess/scratch/PAS1533/smathur/ibd/lists/byPop/${pop}.final172.sample.list > samples/${pop}.shuf8.run${r}.sample.list

# 			vcftools --vcf /fs/ess/scratch/PAS1533/smathur/ibd/geneticLoad/vcf/onlyScat.final172.${type}.phased.recode.vcf \
# 			--freq \
